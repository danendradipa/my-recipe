<script setup lang="ts">
import { RouterLink } from "vue-router";
import type { Recipe } from "../../types/recipe";
import { formatTime } from "../../utils/format";

defineProps<{
  recipe: Recipe;
}>();
</script>

<template>
  <RouterLink
    :to="`/recipes/${recipe.id}`"
    class="block bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300"
  >
    <div class="relative h-48 overflow-hidden">
      <img
        :src="recipe.image"
        :alt="recipe.name"
        class="w-full h-full object-cover hover:scale-110 transition-transform duration-300"
      />
      <div
        class="absolute top-2 right-2 bg-orange-600 text-white px-2 py-1 rounded-md text-sm font-semibold"
      >
        ⭐ {{ recipe.rating }}
      </div>
      <div
        class="absolute bottom-2 left-2 bg-white/90 px-2 py-1 rounded-md text-xs font-medium"
      >
        {{ recipe.difficulty }}
      </div>
    </div>

    <div class="p-4">
      <h3 class="text-lg font-semibold text-gray-800 mb-2 line-clamp-1">
        {{ recipe.name }}
      </h3>

      <p class="text-sm text-gray-600 mb-3">
        {{ recipe.cuisine }} • {{ recipe.mealType.join(", ") }}
      </p>

      <div class="flex items-center justify-between text-sm text-gray-500">
        <div class="flex items-center space-x-1">
          <svg
            class="w-4 h-4"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              stroke-width="2"
              d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"
            />
          </svg>
          <span>
            {{ formatTime(recipe.prepTimeMinutes + recipe.cookTimeMinutes) }}
          </span>
        </div>
        <div class="flex items-center space-x-1">
          <svg
            class="w-4 h-4"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              stroke-width="2"
              d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"
            />
          </svg>
          <span>{{ recipe.servings }} servings</span>
        </div>
      </div>

      <div class="mt-3 flex flex-wrap gap-1">
        <span
          v-for="tag in recipe.tags.slice(0, 3)"
          :key="tag"
          class="px-2 py-1 bg-orange-100 text-orange-600 text-xs rounded-full"
        >
          {{ tag }}
        </span>
      </div>
    </div>
  </RouterLink>
</template>
